...
/**
  * This is an event sourced entity. It has a state, [[MagickeeperState]], which
  ...
  */
class MagickeeperEntity extends PersistentEntity {

  override type Command = MagickeeperCommand[_]
  override type Event = MagickeeperEvent
  override type State = MagickeeperState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: MagickeeperState = MagickeeperState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case MagickeeperState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        MagickeeperState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class MagickeeperState(message: String, timestamp: String)

object MagickeeperState {
  /**
    ...
    */
  implicit val format: Format[MagickeeperState] = Json.format
}

/**
  * This interface defines all the events that the MagickeeperEntity supports.
  */
sealed trait MagickeeperEvent extends AggregateEvent[MagickeeperEvent] {
  def aggregateTag = MagickeeperEvent.Tag
}

object MagickeeperEvent {
  val Tag = AggregateEventTag[MagickeeperEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends MagickeeperEvent
...
/**
  ...
  */
sealed trait MagickeeperCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends MagickeeperCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends MagickeeperCommand[String]
...
/**
  ...
  * application loader.
  */
object MagickeeperSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[MagickeeperState]
  )
}
